...
- **Alpha.18 connection cleanup experiment**: Command-only timeouts now cancel and reset stale raw-HCI setup before retrying, and discovery is limited to TTLock service `1910`. Keep a manual fallback until repeated supervised timing and sequential-command tests pass.
