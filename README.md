...
docker build \
  ...
  --tag tt-lockstar-ha-intergration:0.1.0-alpha.20 \
  ./tt-lockstar-ha-intergration
```
...
- Alpha.20 keeps raw HCI as the default. D-Bus remains selectable for supervised experiments and now paces multipart write-without-response commands after alpha.19 connected quickly but lost the first response.
