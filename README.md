...
- Alpha.56 adds explicit read-only hardware-feature discovery and persists the SDK feature list so auto-lock, sound, PIN, card, passage-mode, and remote-unlock commands do not look unsupported after a direct ESPHome restart. It preserves a fully decoded result when the lock disconnects during the SDK's final connection bookkeeping. Biometric enrollment remains last and unvalidated.
...
docker build \
  ...
  --tag tt-lockstar-ha-intergration:0.1.0-alpha.56 \
  ./tt-lockstar-ha-intergration
```
