...
docker build \
  ...
  --tag tt-lockstar-ha-intergration:0.1.0-alpha.23 \
  ./tt-lockstar-ha-intergration
```
...
- Alpha.23 keeps raw HCI as the default. D-Bus remains selectable for supervised experiments and can refresh only the target's unpaired stale BlueZ cache before waiting for live rediscovery.
