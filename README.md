...
docker build \
  ...
  --tag tt-lockstar-ha-intergration:0.1.0-alpha.24 \
  ./tt-lockstar-ha-intergration
```
...
- Alpha.24 keeps raw HCI as the default. D-Bus remains selectable for supervised experiments, waits up to 15 seconds for a keypad wake advertisement, and can refresh only the target's unpaired stale BlueZ cache before waiting for live rediscovery.
